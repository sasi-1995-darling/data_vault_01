-- dim invoice adjustment

with cte_hub_invoice_adjustment as (
    select * from {{ ref('hub_invoice_adjustment') }}
)
...
, cte_sat_invoice_adjustment_detail__emtk_ebs__renamed as (
    ...
    from cte_sat_invoice_adjustment_detail__emtk_ebs__latest
)

, cte_invoice_adjustment as (
    select
        hub.invoice_adjustment_hk as dim_invoice_adjustment_pk
        , hub.invoice_adjustment_bk
        , sat.org_id
        , sat.trx_number
        ...
        , sat.valid_from
    from cte_hub_invoice_adjustment as hub
        inner join cte_sat_invoice_adjustment_detail__emtk_ebs__renamed as sat
            on hub.invoice_adjustment_hk = sat.invoice_adjustment_hk
)

, cte_default as (
    select
        CAST(MD5_BINARY(-1) as BINARY(16)) as dim_invoice_adjustment_pk
        , null as invoice_adjustment_bk
        , null as org_id
        , null as trx_number
        , null as order_num -- interface_header_attribute1
        , null as order_type -- interface_header_attribute2
