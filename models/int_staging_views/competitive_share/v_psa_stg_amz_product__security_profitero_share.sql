...
---- SRC LAYER ----
WITH
SRC_S              as ( SELECT * FROM {{ source('profitero_share_master_lock', 'dim_amz_product') }} as SRC 
...
SRC_P              as ( SELECT * FROM {{ source('profitero_share_master_lock', 'dim_account_product') }} as SRC
                        -- DEDUP: this is intentional exact-row dedup, NOT business-key dedup — partition on the ENTIRE row (all descriptive attrs) to collapse exact-duplicate reloads from the source. Earliest PSA load wins on true attribute change; identical payloads reduce to one row. 
...
/*
SRC_S              as ( SELECT * FROM PSA_PROD.PROFITERO_SHARE_MASTER_LOCK.DIM_AMZ_PRODUCT )
, SRC_P              as ( SELECT * FROM PSA_PROD.PROFITERO_SHARE_MASTER_LOCK.DIM_ACCOUNT_PRODUCT )
...
, LOGIC_S as (
    SELECT
        coalesce(nullif(trim(AMZ_PRODUCT_ID), ''), '-1')             as                                       ASIN_BK
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.PROFITERO_SHARE_MASTER_LOCK.DIM_AMZ_PRODUCT'
)
