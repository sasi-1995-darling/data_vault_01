---- SRC LAYER ----
WITH
SRC_SRC            as ( SELECT * FROM {{ source('profitero_share_master_lock', 'dim_amz_category') }} as SRC 
...
SRC_ref_bkcc       as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC 
                        WHERE rec_src = 'US.PROFITERO_SHARE_MASTER_LOCK.DIM_AMZ_CATEGORY' )

/*
SRC_SRC            as ( SELECT * FROM profitero_share_master_lock.dim_amz_category )
