...
SRC_S              as ( SELECT DIM_AMZ_PRODUCT_SALE_KEY, DIM_DATE_KEY, DIM_AMZ_PRODUCT_KEY, DIM_AMZ_CATEGORY_KEY, ACCOUNT_PRODUCT_ID, PLATFORM, FIRST_PARTY_SALES, THIRD_PARTY_SALES, TOTAL_SALES, FIRST_PARTY_UNITS, THIRD_PARTY_UNITS, TOTAL_UNITS, REPORTED_IN_ARA, CREATED_AT, UPDATED_AT, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND
                        FROM {{ source('profitero_share_master_lock', 'dim_amz_product_sale') }} as SRC
...
SRC_C              as ( SELECT DIM_AMZ_CATEGORY_KEY, AMZ_CATEGORY_ID, PSA_DELETE_IND
                        FROM {{ source('profitero_share_master_lock', 'dim_amz_category') }} as SRC
...
SRC_D              as ( SELECT DIM_DATE_KEY, DT_DATE
                        FROM {{ source('profitero_share_master_lock', 'dim_date') }} as SRC
                        WHERE PSA_DELETE_IND = 'N' ),
SRC_a              as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S              as ( SELECT * FROM profitero_share_master_lock.dim_amz_product_sale )
SRC_P              as ( SELECT * FROM profitero_share_master_lock.dim_amz_product )
SRC_C              as ( SELECT * FROM profitero_share_master_lock.dim_amz_category )
SRC_D              as ( SELECT * FROM profitero_share_master_lock.dim_date )
...
, LOGIC_a as (
    SELECT
      ...
      , REC_SRC
    FROM SRC_a
    WHERE REC_SRC = 'US.PROFITERO_SHARE_MASTER_LOCK.DIM_AMZ_PRODUCT_SALE'
)
---- RENAME LAYER ----
