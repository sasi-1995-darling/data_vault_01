---- SRC LAYER ----
WITH
SRC_S              as ( SELECT * FROM {{ source('profitero_security', 'sns_products') }} as SRC 
                        where psa_delete_ind='N'
                        qualify row_number() over(partition by asin order by psa_load_dts desc)=1 ),
SRC_a              as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S              as ( SELECT * FROM profitero_security.sns_products )
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.PROFITERO_SECURITY.SNS_PRODUCTS'
)
...
SELECT
        ...
        , MD5_BINARY(UPPER(CONCAT_WS('||',
          COALESCE(NULLIF(TRIM(CAST(CUSTOMER_PRODUCT_ID as VARCHAR)),''), '^^')
