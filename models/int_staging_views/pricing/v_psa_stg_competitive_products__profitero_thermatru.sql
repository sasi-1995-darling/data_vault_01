---- SRC LAYER ----
WITH
SRC_S              as ( SELECT * FROM {{ source('profitero_thermatru', 'products') }} as SRC 
                        where psa_delete_ind='N'
                        qualify row_number() over(partition by id order by psa_load_dts desc)=1 ),
SRC_a              as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  ),
SRC_p              as ( SELECT * FROM {{ source('profitero_thermatru', 'price_availability_history') }} as SRC 
                        where psa_delete_ind='N'
                        qualify row_number() over(partition by product_id, retailer_id order by psa_load_dts desc)=1 ),
SRC_r              as ( SELECT * FROM {{ source('profitero_thermatru', 'retailers') }} as SRC 
                        where psa_delete_ind='N'
                        qualify row_number() over(partition by id order by psa_load_dts desc)=1 )

/*
SRC_S              as ( SELECT * FROM profitero_thermatru.products )
, SRC_a              as ( SELECT * FROM raw_vault.ref_business_key_collision )
, SRC_p              as ( SELECT * FROM profitero_thermatru.price_availability_history )
, SRC_r              as ( SELECT * FROM profitero_thermatru.retailers )
*/
---- LOGIC LAYER ----
