---- SRC LAYER ----
WITH
SRC_S              as ( SELECT DIM_PRODUCT_HISTORY_KEY, DIM_DATE_KEY, DIM_ACCOUNT_PRODUCT_KEY, DIM_RETAILER_KEY, DIM_RETAILER_LOCATION_KEY, AVAILABILITY, REGULAR_PRICE, PROMOTION_TEXT, PROMOTION_PRICE, FIRST_PARTY_WON_BUY_BOX, THIRD_PARTY_SELLER, PRIME_EXCLUSIVE, PROMO_TYPE, UPDATED_AT, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND
                FROM {{ source('profitero_share_master_lock', 'dim_product_history') }} as SRC
                -- Profitero surrogate key reissuance causes multiple DIM_ACCOUNT_PRODUCT_KEY values
                -- for the same logical product, producing fan-out on (date, retailer_location, load_dts).
                -- Dedup here before joins; DIM_PRODUCT_HISTORY_KEY DESC breaks ties deterministically.
                QUALIFY ROW_NUMBER() OVER (
                    PARTITION BY DIM_DATE_KEY, DIM_RETAILER_LOCATION_KEY, PSA_LOAD_DTS
                    ORDER BY UPDATED_AT DESC NULLS LAST, DIM_PRODUCT_HISTORY_KEY DESC
                ) = 1 ),
SRC_a              as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC  ),
SRC_DD             as ( SELECT DIM_DATE_KEY, DT_DATE FROM {{ source('profitero_share_master_lock', 'dim_date') }} as SRC  ),
SRC_AP             as ( SELECT DIM_ACCOUNT_PRODUCT_KEY, ACCOUNT_PRODUCT_ID, UPDATED_AT, PSA_LOAD_DTS, PSA_DELETE_IND
                FROM {{ source('profitero_share_master_lock', 'dim_account_product') }} as SRC
...
SRC_R              as ( SELECT DIM_RETAILER_KEY, RETAILER_NAME, UPDATED_AT, PSA_LOAD_DTS, PSA_DELETE_IND
                FROM {{ source('profitero_share_master_lock', 'dim_retailer') }} as SRC
                QUALIFY NOT (PSA_DELETE_IND = 'Y'
...
, SRC_RL             as ( SELECT DIM_RETAILER_LOCATION_KEY, RETAILER_LOCATION_NAME, UPDATED_AT, PSA_LOAD_DTS, PSA_DELETE_IND
                FROM {{ source('profitero_share_master_lock', 'dim_retailer_location') }} as SRC
                QUALIFY NOT (PSA_DELETE_IND = 'Y'
...
/*
SRC_S              as ( SELECT * FROM profitero_share_master_lock.dim_product_history )
, SRC_a              as ( SELECT * FROM raw_vault.ref_business_key_collision )
, SRC_DD             as ( SELECT * FROM profitero_share_master_lock.dim_date )
, SRC_AP             as ( SELECT * FROM profitero_share_master_lock.dim_account_product )
, SRC_R              as ( SELECT * FROM profitero_share_master_lock.dim_retailer )
*/
---- LOGIC LAYER ----
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.PROFITERO_SECURITY.PRICING_AVAILABILITY_HISTORY'
)
