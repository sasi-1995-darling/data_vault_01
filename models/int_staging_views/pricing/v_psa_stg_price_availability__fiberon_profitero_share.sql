...
SRC_S              as ( SELECT DIM_PRODUCT_HISTORY_KEY, DIM_DATE_KEY, DIM_ACCOUNT_PRODUCT_KEY, DIM_RETAILER_KEY, DIM_RETAILER_LOCATION_KEY, AVAILABILITY, REGULAR_PRICE, PROMOTION_TEXT, PROMOTION_PRICE, FIRST_PARTY_WON_BUY_BOX, THIRD_PARTY_SELLER, PRIME_EXCLUSIVE, PROMO_TYPE, UPDATED_AT, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND
                FROM {{ source('profitero_share_fiberon', 'dim_product_history') }} as SRC
...
SRC_DD             as ( SELECT DIM_DATE_KEY, DT_DATE FROM {{ source('profitero_share_fiberon', 'dim_date') }} as SRC  ),
...
, SRC_RL             as ( SELECT DIM_RETAILER_LOCATION_KEY, RETAILER_LOCATION_NAME, UPDATED_AT, PSA_LOAD_DTS, PSA_DELETE_IND
                FROM {{ source('profitero_share_fiberon', 'dim_retailer_location') }} as SRC
                QUALIFY NOT (PSA_DELETE_IND = 'Y'
...
/*
SRC_S              as ( SELECT * FROM profitero_share_fiberon.dim_product_history )
, SRC_a              as ( SELECT * FROM raw_vault.ref_business_key_collision )
, SRC_DD             as ( SELECT * FROM profitero_share_fiberon.dim_date )
, SRC_AP             as ( SELECT * FROM profitero_share_fiberon.dim_account_product )
, SRC_R              as ( SELECT * FROM profitero_share_fiberon.dim_retailer )
*/
---- LOGIC LAYER ----
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.PROFITERO_FIBERON.PRICING_AVAILABILITY_HISTORY'
)
