---- SRC LAYER ----
WITH
SRC_S              as ( SELECT DIM_RETAILER_PRODUCT_KEY, DIM_RETAILER_KEY, RPC, EAN, UPC, PRODUCT_MODEL, PRODUCT_URL, UPDATED_AT, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND FROM {{ source('profitero_share_fypon', 'dim_retailer_product') }} as SRC
                        where psa_delete_ind='N' ), -- to match the old profitero model logic
SRC_a              as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC  ),
SRC_FAR            as ( SELECT DIM_RETAILER_PRODUCT_KEY, DIM_ACCOUNT_PRODUCT_KEY FROM {{ source('profitero_share_fypon', 'fact_account_retailer_product') }} as SRC
...
SRC_AP             as ( SELECT DIM_ACCOUNT_PRODUCT_KEY, ACCOUNT_PRODUCT_ID, EAN, UPC, PRODUCT_MODEL, UPDATED_AT, PSA_LOAD_DTS, PSA_DELETE_IND
                ...
                FROM {{ source('profitero_share_fypon', 'dim_account_product') }} as SRC
...
SRC_r              as ( SELECT DIM_RETAILER_KEY, RETAILER_NAME, UPDATED_AT, PSA_LOAD_DTS, PSA_DELETE_IND
                    ...
                    */
                    , CASE WHEN PSA_DELETE_IND = 'Y'
                         ...
                         THEN TRUE ELSE FALSE END AS TRUNCATE_RELOAD_FLAG
                FROM {{ source('profitero_share_fypon', 'dim_retailer') }} as SRC
                QUALIFY NOT TRUNCATE_RELOAD_FLAG
...
SRC_AMZ            as ( SELECT AMZ_PRODUCT_ID
                        ...
                        , CASE WHEN PSA_DELETE_IND = 'Y'
                               ...
                               THEN TRUE ELSE FALSE END AS TRUNCATE_RELOAD_FLAG
                        FROM {{ source('profitero_share_fypon', 'dim_amz_product') }} as SRC
                        QUALIFY NOT TRUNCATE_RELOAD_FLAG
                           AND ROW_NUMBER() OVER (PARTITION BY AMZ_PRODUCT_ID ORDER BY UPDATED_AT DESC, PSA_LOAD_DTS DESC) = 1 )

/*
SRC_S              as ( SELECT * FROM profitero_share_fypon.dim_retailer_product )
, SRC_a              as ( SELECT * FROM raw_vault.ref_business_key_collision )
, SRC_FAR            as ( SELECT * FROM profitero_share_fypon.fact_account_retailer_product )
, SRC_AP             as ( SELECT * FROM profitero_share_fypon.dim_account_product )
, SRC_r              as ( SELECT * FROM profitero_share_fypon.dim_retailer )
*/
---- LOGIC LAYER ----
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.PROFITERO_FYPON.PRODUCTS'
)
