...
, SRC_RL             as ( SELECT DIM_RETAILER_LOCATION_KEY, RETAILER_LOCATION_NAME, UPDATED_AT, PSA_LOAD_DTS, PSA_DELETE_IND
                FROM {{ source('profitero_share_fypon', 'dim_retailer_location') }} as SRC
                QUALIFY NOT (PSA_DELETE_IND = 'Y'
...
/*
SRC_S              as ( SELECT * FROM profitero_share_fypon.dim_product_history )
, SRC_a              as ( SELECT * FROM raw_vault.ref_business_key_collision )
, SRC_DD             as ( SELECT * FROM profitero_share_fypon.dim_date )
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.PROFITERO_FYPON.PRICING_AVAILABILITY_HISTORY'
)
