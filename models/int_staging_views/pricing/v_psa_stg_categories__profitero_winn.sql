---- SRC LAYER ----
WITH
SRC_S              as ( SELECT * FROM {{ source('profitero', 'categories_products') }} as SRC 
...
/*
SRC_S              as ( SELECT * FROM profitero.categories_products )
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.PROFITERO_WINN.CATEGORIES_PRODUCTS'
)
