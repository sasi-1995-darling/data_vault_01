---- SRC LAYER ----
WITH
SRC_S              as ( SELECT * FROM {{ source('profitero_fypon', 'categories') }} as SRC 
                        where psa_delete_ind='N'
                        qualify row_number() over(partition by id order by psa_load_dts desc)=1 ),
SRC_a              as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S              as ( SELECT * FROM profitero_fypon.categories )
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.PROFITERO_FYPON.CATEGORIES'
)
