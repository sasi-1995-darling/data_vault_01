---- SRC LAYER ----
WITH
SRC_cosp           as ( SELECT * FROM {{ source('sap_ecc_prd', 'z_cosp') }} as SRC  ),
SRC_A              as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_cosp           as ( SELECT * FROM sap_ecc_prd.z_cosp )
...
, RENAME_cosp as (
  ...
  , coalesce(nullif(trim(VRGNG), ''), '-1')                                                         as                  COST_BUSINESS_TRANSACTION_TYPE_BK
  , coalesce(nullif(trim(VBUND), ''), '-1')                                                         as                                    LEGAL_ENTITY_BK
  , coalesce(nullif(trim(PARGB), ''), '-1')                                                         as                   TRADING_PARTNER_BUSINESS_AREA_BK
  , coalesce(nullif(trim(BEKNZ), ''), '-1')                                                         as                          DEBIT_CREDIT_INDICATOR_BK
  , coalesce(nullif(trim(TWAER), ''), '-1')                                                         as                                        CURRENCY_BK
...
, FILTER_A as (
    SELECT *
    FROM RENAME_A
    WHERE rec_src = 'USOHNO.SAP.ECCPRD.Z_COSP'
)
...
SELECT
      ...
      , MD5_BINARY(UPPER(CONCAT_WS('||',
            COALESCE(NULLIF(TRIM(CAST(COST_BUSINESS_TRANSACTION_TYPE_BK as VARCHAR)),''), '^^')
            , COALESCE(NULLIF(TRIM(CAST(BKCC as VARCHAR)),''), '^^')
        )))                                                          as                  COST_BUSINESS_TRANSACTION_TYPE_HK
      , MD5_BINARY(UPPER(CONCAT_WS('||',
            COALESCE(NULLIF(TRIM(CAST(LEGAL_ENTITY_BK as VARCHAR)),''), '^^')
            , COALESCE(NULLIF(TRIM(CAST(BKCC as VARCHAR)),''), '^^')
        )))                                                          as                                    LEGAL_ENTITY_HK
      , MD5_BINARY(UPPER(CONCAT_WS('||',
            COALESCE(NULLIF(TRIM(CAST(TRADING_PARTNER_BUSINESS_AREA_BK as VARCHAR)),''), '^^')
            , COALESCE(NULLIF(TRIM(CAST(BKCC as VARCHAR)),''), '^^')
        )))                                                          as                                   TRADING_PARTNER_BUSINESS_AREA_HK
      , MD5_BINARY(UPPER(CONCAT_WS('||',
            COALESCE(NULLIF(TRIM(CAST(DEBIT_CREDIT_INDICATOR_BK as VARCHAR)),''), '^^')
            , COALESCE(NULLIF(TRIM(CAST(BKCC as VARCHAR)),''), '^^')
        )))                                                          as                                    DEBIT_CREDIT_INDICATOR_HK
      , MD5_BINARY(UPPER(CONCAT_WS('||',
            COALESCE(NULLIF(TRIM(CAST(CURRENCY_BK as VARCHAR)),''), '^^')
            , COALESCE(NULLIF(TRIM(CAST(BKCC as VARCHAR)),''), '^^')
        )))                                                          as                                        CURRENCY_HK
        , LEDGER_BK
        , OBJECT_NUMBER_BK
        ...
        , COST_ELEMENT_BK
        , ORIGIN_GROUP_BK
        , COST_BUSINESS_TRANSACTION_TYPE_BK
        , LEGAL_ENTITY_BK
        , TRADING_PARTNER_BUSINESS_AREA_BK
        ...
        , COALESCE(NULLIF(TRIM(CAST(PERIOD_BLOCK_BK as VARCHAR)),''), '^^')
        , COALESCE(NULLIF(TRIM(CAST(BKCC as VARCHAR)),''), '^^')
        ))) as COST_TOTALS_FOR_EXTERNAL_POSTINGS_HK
