...
---- SRC LAYER ----
WITH
SRC_a              as ( SELECT GLCHANGETIME, GLDELFLAG, GLREQUEST, GLSOURCESYSTEM, KAPID, KTEXT, MANDT, PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, SPRAS FROM {{ source('sap_ecc_prd', 'z_kakt') }} as SRC 
                        Where SPRAS in ('E', 'I') ),
SRC_bkcc           as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC  )
/*
SRC_a              as ( SELECT * FROM sap_ecc_prd.z_kakt )
...
, FILTER_bkcc as (
    SELECT *
    FROM RENAME_bkcc
    WHERE rec_src = 'USOHNO.SAP.ECCPRD.Z_KAKT'
)
