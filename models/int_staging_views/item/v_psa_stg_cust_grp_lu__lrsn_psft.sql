---- SRC LAYER ----
WITH
SRC_S              as ( SELECT * FROM {{ source('lrsn_psft_sysadm', 'ps_l_cust_grp_lu') }} as SRC 
...
/*
SRC_S              as ( SELECT * FROM lrsn_psft_sysadm.ps_l_cust_grp_lu )
...
, FILTER_A as (
    SELECT *
    FROM RENAME_A
    WHERE rec_src = 'USSDBR.ORCL.PSFTPRD.PS_L_CUST_GRP_LU'
)
...
---- FINAL LAYER ----
SELECT
          CONCAT (L_GRP_TYPE, '||',L_GRP_CODE)                         as CUST_GRP_BK
        , L_GRP_TYPE
        , L_GRP_CODE
        , SETID
        , DESCR
