---- SRC LAYER ----
WITH
SRC_S              as ( SELECT _FILE, _MODIFIED, _FIVETRAN_SYNCED, YEAR_MONTH, DATE, TRANSACTION_TYPE, SELL_LOCATION_ID, LEGACY_SELL_LOCATION_ID, SELL_LOCATION_NAME, SHIP_LOCATION_ID, LEGACY_SHIP_LOCATION_ID, SHIP_LOCATION_NAME, SHIP_LOCATION_CITY, SHIP_LOCATION_STATE, SHIP_LOCATION_ZIP, ORDER_CHANNEL, BUILD_COM_ORDER, CUSTOMER_GROUP, FEI_PRODUCT_NO, FEI_PRODUCT_CODE, FEI_PRODUCT_DESCRIPTION, VENDOR_CODE, SHIPPED_QTY, PRODUCT_DEST_CITY, PRODUCT_DEST_STATE, PRODUCT_DEST_ZIP, UOM, INVOICE, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND, _LINE FROM {{ source('ferguson', 'moen_m_3_a_new') }} as SRC  ),
SRC_A              as ( SELECT REC_SRC, BKCC FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S              as ( SELECT * FROM ferguson.moen_m_3_a_new )
...
, FILTER_A as (
    SELECT *
    FROM RENAME_A
    WHERE rec_src = 'US.EXCEL.FERGUSON.MOEN_M_3_A_NEW'
)
