---- SRC LAYER ----
WITH
SRC_a              as ( SELECT ASIN, END_DATE, GLANCE_VIEWS, MARKETPLACE_ID, PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, START_DATE, _FIVETRAN_SYNCED FROM {{ source('amazon_sp_ft_moen_anaheim', 'vendor_traffic_report_daily') }} as SRC  ),
...
/*
SRC_a              as ( SELECT * FROM AMAZON_SP_FT_MOEN_ANAHEIM.VENDOR_TRAFFIC_REPORT_DAILY )
...
, FILTER_bkcc as (
    SELECT *
    FROM RENAME_bkcc
    WHERE rec_src = 'US.API_FT.AMAZON_SP_FT_MOEN_ANAHEIM.VENDOR_TRAFFIC_REPORT_DAILY'
)
