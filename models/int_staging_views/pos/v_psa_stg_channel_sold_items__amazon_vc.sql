---- SRC LAYER ----
WITH
SRC_a              as ( SELECT ASIN, CUSTOMER_RETURNS, ORDERDED_REVENUE_AMT, ORDERDED_REV_CURRCODE, ORDERDED_UNITS, PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, REPORT_END_DATE, REPORT_START_DATE, RUN_DATE, SHIPPED_COGS_AMT, SHIPPED_COGS_CURRCODE, SHIPPED_REVENUE_AMT, SHIPPED_REV_CURRCODE, SHIPPED_UNITS, VENDORCENTRAL_ACCOUNT FROM {{ source('amazon_vc', 'sales') }} as SRC  ),
...
SRC_b              as ( SELECT ASIN, MODEL_STYLE_NUMBER, MODEL_NUMBER FROM {{ source('amazon_xref_prd', 'amazon_moen_catalog_hist') }} as SRC QUALIFY 1 = ROW_NUMBER() OVER (PARTITION BY ASIN ORDER BY psa_load_dts DESC) ),
...
/*
SRC_a              as ( SELECT * FROM amazon_vc.Sales )
...
, LOGIC_a as (
    SELECT
        'AMAZON'                                                      as                                           STORE_BK
      , REPORT_START_DATE
      , REPORT_START_DATE::DATE AS REPORT_START_DT
      , REPORT_END_DATE
      , REPORT_END_DATE::DATE AS REPORT_END_DT
      , ASIN
      , CUSTOMER_RETURNS
...
SELECT
        ...
        , MD5_BINARY(UPPER(NULLIF(CONCAT(
            ...
            , '||', IFNULL(TRIM(VENDORCENTRAL_ACCOUNT::text), '^^') 
