---- SRC LAYER ----
WITH
SRC_S              as ( SELECT * FROM {{ source('lowes_vpp_psa', 'sales_inventory_larson') }} as SRC  ),
SRC_A              as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S              as ( SELECT * FROM lowes_vpp_psa.sales_inventory_larson )
...
, FILTER_A as (
    SELECT *
    FROM RENAME_A
    WHERE rec_src = 'US.API.LOWES_VPP.SALES_INVENTORY_LARSON'
)
