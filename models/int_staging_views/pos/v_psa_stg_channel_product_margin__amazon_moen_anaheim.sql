---- SRC LAYER ----
WITH
SRC_a              as ( SELECT ASIN, END_DATE, MARKETPLACE_ID, NET_PURE_PRODUCT_MARGIN, PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, START_DATE, _FIVETRAN_SYNCED FROM {{ source('amazon_sp_ft_moen_anaheim', 'net_pure_product_margin_report_daily') }} as SRC  ),
...
/*
SRC_a              as ( SELECT * FROM amazon_sp_ft_moen_anaheim.net_pure_product_margin_report_daily )
...
, FILTER_bkcc as (
    SELECT *
    FROM RENAME_bkcc
    WHERE rec_src = 'US.API_FT.AMAZON_SP_FT_MOEN_ANAHEIM.NET_PURE_PRODUCT_MARGIN_REPORT_DAILY'
)
