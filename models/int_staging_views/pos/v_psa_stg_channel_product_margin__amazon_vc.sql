---- SRC LAYER ----
WITH
SRC_a              as ( SELECT REPORT_START_DATE, REPORT_END_DATE, ASIN, NETPPM, RUN_DATE, VENDORCENTRAL_ACCOUNT, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND
                        FROM {{ source('amazon_vc', 'netppm') }} as SRC  ),
...
/*
SRC_a              as ( SELECT * FROM amazon_vc.netppm )
...
, LOGIC_a as (
    SELECT
        REPORT_START_DATE::DATE                                      as                                           REPORT_START_DATE
      , REPORT_END_DATE::DATE                                        as                                           REPORT_END_DATE
      , ASIN
      , NETPPM
      , RUN_DATE
...
, FILTER_bkcc as (
    SELECT *
    FROM RENAME_bkcc
    WHERE rec_src = 'US.API.AMAZON_VC.NETPPM'
)
