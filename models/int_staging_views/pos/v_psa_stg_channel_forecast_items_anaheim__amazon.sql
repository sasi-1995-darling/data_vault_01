---- SRC LAYER ----
WITH
SRC_a              as ( SELECT ASIN, END_DATE, FORECAST_GENERATION_DATE, MARKETPLACE_ID, MEAN_FORECAST_UNITS, PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, P_70_FORECAST_UNITS, P_80_FORECAST_UNITS, P_90_FORECAST_UNITS, START_DATE, _FIVETRAN_SYNCED FROM {{ source('amazon_sp_ft_moen_anaheim', 'vendor_forecasting_retail_report') }} as SRC  ),
...
/*
SRC_a              as ( SELECT * FROM AMAZON_SP_FT_MOEN_ANAHEIM.VENDOR_FORECASTING_RETAIL_REPORT )
...
, FILTER_bkcc as (
    SELECT *
    FROM RENAME_bkcc
    WHERE rec_src = 'US.API_FT.AMAZON_SP_FT_MOEN_ANAHEIM.VENDOR_FORECASTING_RETAIL_REPORT'
)
