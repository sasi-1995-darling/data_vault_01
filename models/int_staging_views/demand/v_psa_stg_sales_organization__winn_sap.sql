---- SRC LAYER ----
WITH
SRC_a              as ( SELECT VKORG, MANDT, SPRAS, GLREQUEST, VTEXT, GLDELFLAG, GLCHANGETIME, GLSOURCESYSTEM, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND 
                        FROM {{ source('sap_ecc_prd', 'z_tvkot') }} as SRC  ),
SRC_bkcc           as ( SELECT REC_SRC, BKCC FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_a              as ( SELECT * FROM sap_ecc_prd.z_tvkot )
...
, FILTER_bkcc as (
    SELECT *
    FROM RENAME_bkcc
    WHERE rec_src = 'USOHNO.SAP.ECCPRD.Z_TVKOT'
)
