{{
    config(
        ...
        tags=['historical', 'one_time_load']
    )
}}
---- SRC LAYER ----
WITH
SRC_s              as ( SELECT ARTNR, AUART, AUGRU, BELNR, BUDAT, BUKRS, BZIRK, COUNTER1, COUNTER2, CURTYPE, CUST_SALES, FADAT, FENUM, FKART, GJAHR, GSBER, KAUFN, KDPOS, KNDNR, 
                               KOKRS, KSPOS, KSTAR, KSTRG, KTOPL, KURSF, KVGR1, KVGR2, KVGR3, MANDT, PALEDGER, PAOBJNR, PASUBNR, PERIO, PERIV, POSNR, PPRCTR, PRCTR, PRZNR, 
                               PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSPNR, PSTYV, QMNUM, RBELN, REC_WAERS, RKAUFNR, RPOSN, SKOST, SPART, STO_BELNR, STO_POSNR, 
                               TCTVAPRCTP, VALUTYP, VERSI, VKBUR, VKGRP, VKORG, VRGAR, VTWEG, VVAAL, VVACD, VVACL, VVACQ, VVADM, VVADV, VVAFC, VVAGM, VVANS, VVART, VVBLD, 
                               VVBON, VVCBA, VVCDR, VVCES, VVCJA, VVCJP, VVCMB, VVCMG, VVCMR, VVCMW, VVCOP, VVCRL, VVCRR, VVCST, VVDCI, VVDEF, VVDEV, VVDFR, VVDIP, VVDSA, 
                               VVDSP, VVEDI, VVERR, VVERW, VVFPP, VVFRA, VVFRC, VVFRM, VVFRO, VVGBP, VVGIS, VVGMG, VVGRI, VVGRR, VVGRS, VVHCD, VVHDL, VVHFR, VVICP, VVIND, 
                               VVLTS, VVMIN, VVMKF, VVMKT, VVMKV, VVMOD, VVMWP, VVNCD, VVNET, VVNSA, VVOCC, VVOIP, VVOPN, VVOTC, VVOVR, VVPRD, VVPRO, VVPRP, VVPRT, VVPRV, 
                               VVQTY, VVQTY_ME, VVRCD, VVRET, VVRFC, VVRND, VVRST, VVSBD, VVSBK, VVSBN, VVSDP, VVSEF, VVSEV, VVSHW, VVSIP, VVSOD, VVSPC, VVSPD, VVSPS, VVSSA, 
                               VVSTS, VVSUP, VVTHR, VVTRC, VVTRK, VVTSD, VVVBR, VVVBW, VVVIP, VVVLR, VVVMW, VVVPO, VVVPP, VVWHS, WADAT, WERKS, WRTTP, WWCDV, WWCMP, WWDEF, 
                               WWDSP, WWDTC, WWDTS, WWDTZ, WWKNU, WWPBT, WWPCA, WWPCS, WWPFA, WWPFE, WWPFF, WWPFS, WWPII, WWPIP, WWPPY, WWPSH, WWPSL, WWPSR, WWPSS, WWRSN, 
                               WWSTC, WWSTP, WWSTS, WWSTZ, ZCMRCAT, ZEXTRACTDATE, ZKURSF, ZPTYPE, ZWORKDAY, ZZDTP, ZZFIN, ZZHAN, ZZLIN, ZZMAJ, ZZMG1, ZZMIN, ZZORC, ZZPMI, 
                               ZZROM, ZZSEG, ZZTYP FROM {{ source('sap_bw', 'azcopa') }} as SRC 
                               {% if not is_incremental() %} 
                               WHERE  GJAHR >= 2019 AND GJAHR <= 2023
                               {% else %}
                                WHERE  GJAHR > 2099
                                {% endif %} 

                        /*This filter is to remove any extraneous historical COPA data that falls outside of the desired MDP fiscal year range in order to 
                        improve performance and efficiency of downstream objects*/ ),
SRC_a              as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_s              as ( SELECT * FROM sap_bw.azcopa )
...
, LOGIC_s as (
    SELECT
      ...
      , PSA_RECORD_SOURCE
      , PSA_DELETE_IND
      , CONVERT_TIMEZONE('UTC', TO_TIMESTAMP(COALESCE(NULLIF(ZEXTRACTDATE, ' '), PSA_LOAD_DTS))) as                                           LOAD_DTS
      , VERSI                                                        as                                            S_VERSI
      , POSNR                                                        as                                            S_POSNR
    FROM SRC_s
)
...
SELECT
        ...
        , MD5_BINARY(UPPER(NULLIF(CONCAT(
            ...
            , '||', IFNULL(TRIM(ZKURSF::text), '^^') 
            , '||', IFNULL(TRIM(COUNTER1::text), '^^') 
            , '||', IFNULL(TRIM(COUNTER2::text), '^^') 
            , '||', IFNULL(TRIM(VVPRT::text), '^^') 
            , '||', IFNULL(TRIM(CUST_SALES::text), '^^') 
