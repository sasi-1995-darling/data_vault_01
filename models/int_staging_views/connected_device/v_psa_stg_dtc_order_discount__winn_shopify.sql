---- SRC LAYER ----
WITH
SRC_D1             as ( SELECT ALLOCATION_METHOD, CODE, DESCRIPTION, INDEX, ORDER_ID, PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, TARGET_SELECTION, TARGET_TYPE, TITLE, TYPE, VALUE, VALUE_TYPE, _FIVETRAN_SYNCED
                        ...
                        FROM {{ source('shopify_moen', 'discount_application') }} as SRC
...
/*
SRC_D1             as ( SELECT * FROM SHOPIFY_MOEN.DISCOUNT_APPLICATION )
...
, FILTER_A1 as (
    SELECT *
    FROM RENAME_A1
    WHERE rec_src = 'US.SHOPIFY_MOEN.DISCOUNT_APPLICATION'
)
...
SELECT
        ...
        , MD5_BINARY(UPPER(NULLIF(CONCAT(
            ...
            , '||', IFNULL(TRIM(CODE::text), '^^') 
            , '||', IFNULL(TRIM(DESCRIPTION::text), '^^') 
            , '||', IFNULL(TRIM(VALUE::text), '^^') 
            , '||', IFNULL(TRIM(VALUE_TYPE::text), '^^') 
            , '||', IFNULL(TRIM(ALLOCATION_METHOD::text), '^^') 
            , '||', IFNULL(TRIM(TARGET_SELECTION::text), '^^') 
            , '||', IFNULL(TRIM(TARGET_TYPE::text), '^^') 
