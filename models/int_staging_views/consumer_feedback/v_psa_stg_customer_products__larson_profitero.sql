---- SRC LAYER ----
WITH
SRC_S1             as ( SELECT * FROM {{ source('profitero_larson_psa', 'customer_products') }} as SRC  ),
SRC_A1             as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S1             as ( SELECT * FROM profitero_larson_psa.customer_products )
...
, FILTER_A1 as (
    SELECT *
    FROM RENAME_A1
    WHERE rec_src = 'US.PROFITERO_LARSON.CUSTOMER_PRODUCTS'
)
