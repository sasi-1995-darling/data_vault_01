---- SRC LAYER ----
WITH
SRC_S1             as ( SELECT * FROM {{ source('profitero_winn_psa', 'reviews') }} as SRC 
...
SRC_S3             as ( SELECT ID, NAME FROM {{ source('profitero_winn_psa', 'retailers') }} as SRC 
                        QUALIFY (ROW_NUMBER() OVER(PARTITION BY ID ORDER BY UPDATED_AT DESC ))=1 )

/*
SRC_S1             as ( SELECT * FROM profitero_winn_psa.reviews )
