---- SRC LAYER ----
WITH
SRC_S1             as ( SELECT * FROM {{ source('profitero_fiberon_psa', 'product_ratings') }} as SRC 
...
SRC_S3             as ( SELECT * FROM {{ source('profitero_fiberon_psa', 'retailers') }} as SRC 
                        QUALIFY (ROW_NUMBER() OVER(PARTITION BY ID ORDER BY UPDATED_AT DESC, PSA_LOAD_DTS DESC  ))=1 )

/*
SRC_S1             as ( SELECT * FROM profitero_fiberon_psa.product_ratings )
SRC_a              as ( SELECT * FROM raw_vault.ref_business_key_collision )
SRC_S3             as ( SELECT * FROM profitero_fiberon_psa.retailers )
*/
---- LOGIC LAYER ----
