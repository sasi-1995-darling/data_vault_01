---- SRC LAYER ----
WITH
SRC_S1             as ( SELECT PRODUCT_ID, BRAND_ID, REVIEW_SOURCE_ID, REVIEW_ID, REVIEW_TITLE, REVIEW_TEXT, DATE, PRODUCT_TITLE, SUBCATEGORY_LABEL, SUBCATEGORY_SENTENCE, SENTIMENT, CREATED_AT, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND, HS_FLAG FROM {{ source('sentiment_psa', 'sentiment_subcategory_mapping') }} as SRC  ),
SRC_A1             as ( SELECT REC_SRC, BKCC FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S1             as ( SELECT * FROM sentiment_psa.sentiment_subcategory_mapping )
...
, FILTER_A1 as (
    SELECT *
    FROM RENAME_A1
    WHERE rec_src = 'FBIN.DATASCIENCE.SENTIMENT.SENTIMENT_SUBCATEGORY_MAPPING'
)
