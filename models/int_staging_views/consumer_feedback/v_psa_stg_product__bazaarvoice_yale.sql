---- SRC LAYER ----
WITH
SRC_S1             as ( SELECT * FROM {{ source('ft_bazaarvoice_yale', 'product') }} as SRC  ),
SRC_A1             as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S1             as ( SELECT * FROM ft_bazaarvoice_yale.product )
...
, FILTER_A1 as (
    SELECT *
    FROM RENAME_A1
    WHERE rec_src = 'US.BAZAARVOICE_YALE.PRODUCT'
)
