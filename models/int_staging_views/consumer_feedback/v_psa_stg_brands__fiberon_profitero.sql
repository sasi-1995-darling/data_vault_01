---- SRC LAYER ----
WITH
SRC_S1             as ( SELECT * FROM {{ source('profitero_fiberon_psa', 'brands') }} as SRC 
                        where psa_delete_ind = 'N'
                        QUALIFY (ROW_NUMBER() OVER(PARTITION BY ID ORDER BY UPDATED_AT DESC, PSA_LOAD_DTS DESC ))=1 ),
SRC_C1             as ( SELECT * FROM {{ source('profitero_fiberon_psa', 'customer_products') }} as SRC 
                        where psa_delete_ind = 'N'
                        QUALIFY (ROW_NUMBER() OVER(PARTITION BY ID ORDER BY UPDATED_AT DESC, PSA_LOAD_DTS DESC ))=1 ),
SRC_A1             as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  )

/*
SRC_S1             as ( SELECT * FROM profitero_fiberon_psa.brands )
, SRC_C1             as ( SELECT * FROM profitero_fiberon_psa.customer_products )
...
, FILTER_A1 as (
    SELECT *
    FROM RENAME_A1
    WHERE rec_src = 'US.PROFITERO_FIBERON.BRANDS'
)
