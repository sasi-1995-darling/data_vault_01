...
SRC_s              as ( SELECT DATE, ENGAGED_SESSIONS, ENGAGEMENT_RATE, EVENTS_PER_SESSION, EVENT_COUNT, KEY_EVENTS, PROPERTY, PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, 
                        ...
                        FROM {{ source('google_analytics_4_moen_dtc', 'traffic_acquisition_session_default_channel_grouping_report') }} as SRC 
                        ...
                        qualify 1 = row_number() over(partition by  property, session_default_channel_grouping, date order by psa_load_dts desc)  ),
SRC_a              as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC  ),
SRC_p              as ( SELECT DISPLAY_NAME, NAME FROM {{ source('google_analytics_4_moen_dtc', 'properties') }} as SRC 
...
/*
SRC_s              as ( SELECT * FROM google_analytics_4_moen_dtc.traffic_acquisition_session_default_channel_grouping_report )
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.API.GOOGLE_ANALYTICS_WINN_DTC.TRAFFIC_ACQUISITION_SESSION_DEFAULT_CHANNEL_GROUPING_REPORT'
)
