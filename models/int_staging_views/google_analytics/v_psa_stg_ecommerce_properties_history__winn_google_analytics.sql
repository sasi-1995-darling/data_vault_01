---- SRC LAYER ----
WITH
SRC_p              as ( SELECT ACCOUNT, CREATE_TIME, CURRENCY_CODE, DELETE_TIME, DISPLAY_NAME, EXPIRE_TIME, INDUSTRY_CATEGORY, NAME, PARENT, PROPERTY_TYPE, PSA_DELETE_IND, 
                        PSA_LOAD_DTS, PSA_RECORD_SOURCE, SERVICE_LEVEL, TIME_ZONE, UPDATE_TIME, _FIVETRAN_SYNCED 
                        FROM {{ source('google_analytics_4_moen_dtc_historical', 'properties') }} as SRC
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'US.API.GOOGLE_ANALYTICS_WINN_DTC_HISTORICAL.PROPERTIES'
)
