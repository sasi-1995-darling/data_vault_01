...
SRC_p              as ( SELECT ORGANIZATION_CODE, ORGANIZATION_ID FROM {{ source('emtk_ebs_inv', 'mtl_parameters') }} as SRC 
...
/*
SRC_irml           as ( SELECT * FROM emtk_ebs_inv.mtl_material_transactions )
SRC_a              as ( SELECT * FROM raw_vault.ref_business_key_collision )
SRC_itm            as ( SELECT * FROM emtk_ebs_inv.mtl_system_items_b )
SRC_rcv            as ( SELECT * FROM emtk_ebs_po.rcv_transactions )
SRC_po             as ( SELECT * FROM emtk_ebs_po.po_lines_all )
SRC_sup            as ( SELECT * FROM emtk_ebs_ap.ap_suppliers )
SRC_p              as ( SELECT * FROM emtk_ebs_inv.mtl_parameters )
...
, LOGIC_org as (
    SELECT
      ...
      , ORGANIZATION_ID                                              as                                ORG_ORGANIZATION_ID
    FROM SRC_org
)
---- RENAME LAYER ----
...
, FILTER_a as (
    SELECT *
    FROM RENAME_a
    WHERE rec_src = 'USWIOC.ORCL.EBSEMTK.MTL_MATERIAL_TRANSACTIONS'
)
...
, JOIN_RESULT as (
    ...
    LEFT JOIN FILTER_itm
        ON FILTER_irml.INVENTORY_ITEM_ID = FILTER_itm.ITM_INVENTORY_ITEM_ID
    LEFT JOIN FILTER_rcv
        ON FILTER_irml.RCV_TRANSACTION_ID = FILTER_rcv.RCV_RCV_TRANSACTION_ID
    LEFT JOIN FILTER_p
        ON FILTER_irml.ORGANIZATION_ID = FILTER_p.P_ORGANIZATION_ID
    LEFT JOIN FILTER_po
        ON FILTER_rcv.PO_LINE_ID = FILTER_po.PO_PO_LINE_ID
    LEFT JOIN FILTER_sup
        ON FILTER_rcv.VENDOR_ID = FILTER_sup.SUP_VENDOR_ID
    LEFT JOIN FILTER_org
        ON FILTER_po.PO_ORG_ID = FILTER_org.ORG_ORGANIZATION_ID
)
