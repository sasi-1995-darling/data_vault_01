---- SRC LAYER ----
WITH
SRC_posup          as ( SELECT * FROM {{ source('mlc_ascp', 'msc_supplies') }} as SRC 
                        /*Filter the Open Orders, order_type = 1  */
                        where order_type = 1 ),
SRC_a              as ( SELECT * FROM {{ ref('ref_business_key_collision') }} as SRC  ),
SRC_polml          as ( SELECT * FROM {{ source('ml_ebs_po', 'po_lines_all') }} as SRC 
                        qualify 1= row_number()over(partition by po_line_id order by _fivetran_synced desc, psa_load_dts desc) )

/*
SRC_posup          as ( SELECT * FROM mlc_ascp.msc_supplies )
...
, LOGIC_polml as (
    SELECT
      ...
      , PO_HEADER_ID
      , LINE_NUM
      , PO_LINE_ID                                                   as                                   POLML_PO_LINE_ID
    FROM SRC_polml
)
---- RENAME LAYER ----
...
, JOIN_RESULT as (
    ...
    INNER JOIN FILTER_a
        ON '1' = '1'
    LEFT JOIN FILTER_polml
        ON FILTER_posup.PO_LINE_ID = FILTER_polml.POLML_PO_LINE_ID
)
