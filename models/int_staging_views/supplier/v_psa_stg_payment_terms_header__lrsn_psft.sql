---- SRC LAYER ----
WITH
SRC_SRC            as ( SELECT * FROM {{ source('lrsn_psft_sysadm', 'ps_pymt_trms_hdr') }} as SRC  ),
SRC_ref_bkcc       as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC 
                        WHERE rec_src = 'USSDBR.ORCL.PSFTPRD.PS_PYMT_TRMS_HDR' )

/*
SRC_SRC            as ( SELECT * FROM lrsn_psft_sysadm.ps_pymt_trms_hdr )
...
SELECT
        ...
        , MD5_BINARY(UPPER(NULLIF(CONCAT(
            ...
            , '||', IFNULL(TRIM(PYMNT_TERMS_TYPE::text), '^^') 
