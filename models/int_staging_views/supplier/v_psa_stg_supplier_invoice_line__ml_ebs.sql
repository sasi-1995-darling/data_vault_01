---- SRC LAYER ----
WITH
SRC_SRC            as ( SELECT * FROM {{ source('ml_ebs_ap', 'ap_invoice_lines_all') }} as SRC  ),
SRC_ref_bkcc       as ( SELECT BKCC, REC_SRC FROM {{ ref('ref_business_key_collision') }} as SRC 
                        WHERE rec_src = 'USWIOC.ORCL.EBSPRD.AP_INVOICE_LINES_ALL' )

/*
SRC_SRC            as ( SELECT * FROM ml_ebs_ap.ap_invoice_lines_all )
