...
SRC_a              as ( SELECT * FROM {{ source('sap_ecc_prd', 'z_dd07t') }} as SRC 
                        WHERE DOMNAME = 'PAART' -- Filters results to the 'PAART' domain. In SAP, this domain typically holds values for the "PLANNED ORDER TYPE DESCRIPTIONS"
                        AND DDLANGUAGE = 'E' ), -- This 'E' filter ensures that we retrieve the "English descriptions or texts" for the 'PAART' domain values.
