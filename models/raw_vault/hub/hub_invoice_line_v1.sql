...
SRC_SINVLML        as ( SELECT * FROM {{ ref('v_psa_stg_invoice_line__ml_ebs') }} as SRC 
                        QUALIFY (ROW_NUMBER() OVER(PARTITION BY INVOICE_LINE_HK ORDER BY LOAD_DTS ))=1 ),
...
SRC_SINVLNFB       as ( SELECT * FROM {{ ref('v_psa_stg_invoice_line__fib_ocf') }} as SRC 
                        QUALIFY (ROW_NUMBER() OVER(PARTITION BY INVOICE_LINE_HK ORDER BY LOAD_DTS ))=1 ),
SRC_SSINVLWS       as ( SELECT * FROM {{ ref('v_psa_stg_sales_invoice_line__winn_sap') }} as SRC 
                        QUALIFY (ROW_NUMBER() OVER(PARTITION BY SALES_INVOICE_LINE_HK ORDER BY LOAD_DTS ))=1 )
...
, LOGIC_SSINVLWS as (
    SELECT
      ...
      , SALES_INVOICE_LINE_BK AS INVOICE_LINE_BK
      , BKCC
      , LOAD_DTS
      , REC_SRC
    FROM SRC_SSINVLWS
...
, FILTER_SGLDISTML as (
    ...
    WHERE INVOICE_LINE_HK NOT IN (
        SELECT INVOICE_LINE_HK
        FROM FILTER_SINVLML)
)
