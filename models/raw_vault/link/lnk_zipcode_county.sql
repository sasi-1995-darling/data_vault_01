...
SRC_zip_county     as ( SELECT * FROM {{ ref('v_psa_stg_zip_county__snfl_usps') }} as SRC 
                        QUALIFY (ROW_NUMBER() OVER(PARTITION BY LNK_ZIP_COUNTY_HK ORDER BY LOAD_DTS ))=1 ),
...
, FILTER_county_zip as (
    ...
    WHERE LNK_ZIP_COUNTY_HK NOT IN (
        SELECT LNK_ZIP_COUNTY_HK
...
union all
SELECT 
MD5_BINARY(GR.VALUE) as LNK_ZIP_COUNTY_HK
, MD5_BINARY(GR.VALUE) as ZIP_HK
, MD5_BINARY(GR.VALUE) as COUNTY_HK
