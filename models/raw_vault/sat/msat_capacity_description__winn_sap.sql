---- SRC LAYER ----
WITH
SRC_CAP            as ( SELECT CAPACITY_HK, KAPID, MANDT, SPRAS, GLREQUEST, KTEXT, GLDELFLAG, GLSOURCESYSTEM, GLCHANGETIME, PSA_LOAD_DTS, PSA_RECORD_SOURCE, PSA_DELETE_IND, LOAD_DTS, REC_SRC,HASHDIFF  FROM {{ ref('v_psa_stg_capacity_description__winn_sap') }} as SRC 
                        {% if is_incremental() %}
