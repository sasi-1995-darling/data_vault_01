---- SRC LAYER ----
WITH
SRC_SSA            as ( SELECT * FROM {{ ref('v_psa_stg_sales_sentrysafe__amazon_fivetran') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  )

/*
SRC_SSA            as ( SELECT * FROM STAGING.v_psa_stg_sales_sentrysafe__amazon_fivetran )
*/
---- LOGIC LAYER ----
...
SELECT MD5_BINARY(GR.VALUE::varchar) AS STORE_HK
	...
	, CAST(0 AS NUMBER(38,0)) AS CUSTOMER_RETURNS
