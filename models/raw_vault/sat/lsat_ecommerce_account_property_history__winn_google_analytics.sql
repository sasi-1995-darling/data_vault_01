---- SRC LAYER ----
WITH
SRC_GAEP           as ( SELECT * FROM {{ ref('v_psa_stg_ecommerce_properties_history__winn_google_analytics') }} as SRC 
                        {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %} )

/*
SRC_GAEP           as ( SELECT * FROM STAGING.v_psa_stg_ecommerce_properties_history__winn_google_analytics )
*/
---- LOGIC LAYER ----
