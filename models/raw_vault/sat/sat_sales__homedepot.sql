---- SRC LAYER ----
WITH
SRC_SHD            as ( SELECT * FROM {{ ref('v_psa_stg_sales_homedepot') }} as SRC 
                         {% if is_incremental() %}
...
qualify 1=row_number() over(partition by day, store_hk,d_store_nbr,manuf_part_number,fulfillment_channel,merch_vendor,sku_nbr,sku_status,home_depot_account,hashdiff order by psa_load_dts)
union all
