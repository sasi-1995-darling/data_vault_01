---- SRC LAYER ----
WITH
SRC_OH             as ( SELECT * FROM {{ ref('v_psa_stg_business_data__winn_sap') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', -1, MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  )

/*
SRC_OH             as ( SELECT * FROM staging.v_psa_stg_business_data__winn_sap )
*/
---- LOGIC LAYER ----
