...
, RENAME_po_line as (
    SELECT
      ...
      , BKCC
      , HASHDIFF
      , hash(* exclude(HASHDIFF,LOAD_DTS,_FIVETRAN_ID, _FIVETRAN_SYNCED, LNK_PO_RECEIPT_HK)) as rec_hash
    FROM LOGIC_po_line
)
---- FILTER LAYER ----
...
qualify 1 = row_number() over (partition by PO_NUMBER, ITEM_NO, REL_NUMB, VEND_CODE, PART_CODE, COST_CTR , rec_hash order by PSA_LOAD_DTS)
