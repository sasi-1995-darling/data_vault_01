---- SRC LAYER ----
WITH
SRC_SSM            as ( SELECT * FROM {{ ref('v_psa_stg_store_history__menards') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  )

/*
SRC_SSM            as ( SELECT * FROM STAGING.v_psa_stg_store_history__menards )
*/
---- LOGIC LAYER ----
...
qualify 1=row_number() over(partition by store_hk, load_dts,hashdiff order by psa_load_dts desc)
union all
