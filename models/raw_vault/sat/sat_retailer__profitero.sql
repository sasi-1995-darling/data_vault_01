---- SRC LAYER ----
WITH
SRC_S1             as ( SELECT * FROM {{ ref('v_psa_stg_retailers__winn_profitero') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  ),
SRC_S2             as ( SELECT * FROM {{ ref('v_psa_stg_retailers__security_profitero') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  ),
SRC_S3             as ( SELECT * FROM {{ ref('v_psa_stg_retailers__fypon_profitero') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  ),
SRC_S4             as ( SELECT * FROM {{ ref('v_psa_stg_retailers__fiberon_profitero') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  ),
SRC_S5             as ( SELECT * FROM {{ ref('v_psa_stg_retailers__thermatru_profitero') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  ),
SRC_S6             as ( SELECT * FROM {{ ref('v_psa_stg_retailers__larson_profitero') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  )
...
SELECT 
	...
	, 'US.PROFITERO_WINN.REVIEW' AS REC_SRC
