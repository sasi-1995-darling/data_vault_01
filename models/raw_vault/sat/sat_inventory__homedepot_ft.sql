---- SRC LAYER ----
WITH
SRC_SHD            as ( SELECT * FROM {{ ref('v_psa_stg_inventory__homedepot_ft') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  )

/*
SRC_SHD            as ( SELECT * FROM STAGING.v_psa_stg_inventory__homedepot_ft )
*/
---- LOGIC LAYER ----
...
qualify 1=row_number() over(partition by day, store_hk,d_store_nbr,manuf_part_number,merch_vendor,sku_nbr,sku_status,home_depot_account,hashdiff order by load_dts desc)
union all
