---- SRC LAYER ----
WITH
SRC_SWINN          as ( SELECT * FROM {{ ref('v_psa_stg_controlling_cost_element__winn_sap') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  )

/*
SRC_SWINN          as ( SELECT * FROM STAGING.V_PSA_STG_CONTROLLING_COST_ELEMENT__WINN_SAP )
*/
---- LOGIC LAYER ----

, LOGIC_SWINN as (
    SELECT
        CONTROLLING_AREA_COST_ELEMENT_HK
      , CONTROLLING_AREA_HK
      , COST_ELEMENT_HK
      , VALID_DATE_TO_HK
      , MANDT
...
qualify 1= row_number()over(partition by CONTROLLING_AREA_COST_ELEMENT_HK, HASHDIFF order by LOAD_DTS)
union all
    SELECT        
    MD5_BINARY(GR.VALUE) AS CONTROLLING_AREA_COST_ELEMENT_HK
   , MD5_BINARY(GR.VALUE) AS CONTROLLING_AREA_HK
   , MD5_BINARY(GR.VALUE) AS  COST_ELEMENT_HK    
  , MD5_BINARY(GR.VALUE) AS  VALID_DATE_TO_HK
...
, CAST(NULL AS STRING) AS KATYP
, CAST(NULL AS STRING) AS ERSDA
, CAST(NULL AS STRING) AS USNAM
, CAST(NULL AS STRING) AS EIGEN
, CAST(NULL AS STRING) AS PLAZU
, CAST(NULL AS STRING) AS PLAOR
, CAST(NULL AS STRING) AS PLAUS
, CAST(NULL AS STRING) AS KOSTL
, CAST(NULL AS STRING) AS AUFNR
, CAST(NULL AS STRING) AS MGEFL
, CAST(NULL AS STRING) AS MSEHI
, CAST(NULL AS STRING) AS DEAKT
, CAST(NULL AS STRING) AS LOEVM
