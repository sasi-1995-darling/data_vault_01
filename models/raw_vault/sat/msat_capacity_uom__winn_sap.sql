---- SRC LAYER ----
WITH
SRC_s              as ( SELECT * FROM {{ ref('v_psa_stg_capacity_uom__winn_sap') }} as SRC 
                        {% if is_incremental() %}
                        WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}}){% endif %} )

/*
SRC_s              as ( SELECT * FROM STAGING.V_PSA_STG_CAPACITY_UOM__WINN_SAP )
*/
---- LOGIC LAYER ----
...
WHERE NOT EXISTS (
    SELECT 1 
    FROM {{ this }} existing
    WHERE existing.CAPACITY_HK = JOIN_RESULT.CAPACITY_HK  AND existing.HASHDIFF = JOIN_RESULT.HASHDIFF
)
{% endif %}
