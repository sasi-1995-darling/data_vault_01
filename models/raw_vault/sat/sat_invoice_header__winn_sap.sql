...
SRC_SWINN AS (
    SELECT *
    FROM {{ ref('v_psa_stg_sales_invoice__winn_sap') }} AS SRC
    {% if is_incremental() %}
...
qualify 1= row_number()over(partition by INVOICE_HK, HASHDIFF order by LOAD_DTS)
