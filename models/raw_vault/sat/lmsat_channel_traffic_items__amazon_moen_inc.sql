---- SRC LAYER ----
WITH
SRC_s              as ( SELECT ASIN, END_DATE, GLANCE_VIEWS, HASHDIFF, LOAD_DTS, MARKETPLACE_ID, PSA_DELETE_IND, PSA_LOAD_DTS, PSA_RECORD_SOURCE, REC_SRC, START_DATE, CHANNEL_TRAFFIC_ITEM_LHK, _FIVETRAN_SYNCED FROM {{ ref('v_psa_stg_channel_traffic_items__amazon_moen_inc') }} as SRC 
                        {% if is_incremental() %}
                        WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}}){% endif %} )

/*
SRC_s              as ( SELECT * FROM STAGING.v_psa_stg_channel_traffic_items__amazon_moen_inc )
*/
---- LOGIC LAYER ----
