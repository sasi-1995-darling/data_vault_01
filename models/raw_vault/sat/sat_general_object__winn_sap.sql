---- SRC LAYER ----
WITH
SRC_SWINN          as ( SELECT * FROM {{ ref('v_psa_stg_general_object__winn_sap') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  )

/*
SRC_SWINN          as ( SELECT * FROM STAGING.V_PSA_STG_GENERAL_OBJECT__WINN_SAP )
*/
---- LOGIC LAYER ----
...
qualify 1= row_number()over(partition by OBJECT_NUMBER_HK, HASHDIFF order by LOAD_DTS)
union all
    SELECT        
