...
---- SRC LAYER ----
WITH
SRC_PROD           as ( SELECT * FROM {{ ref('v_psa_stg_amz_product__security_profitero_share') }} as SRC 
                        {% if is_incremental() %}
                              where src.load_dts > (select dateadd('HOUR',-1,max(load_dts)) from {{ this }})
                            {% endif %}   )

/*
SRC_PROD           as ( SELECT * FROM STAGING.v_psa_stg_amz_product__security_profitero_share )
*/
---- LOGIC LAYER ----
