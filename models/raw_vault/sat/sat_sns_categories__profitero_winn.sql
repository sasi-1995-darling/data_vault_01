---- SRC LAYER ----
WITH
SRC_CATWINN        as ( SELECT * FROM {{ ref('v_psa_stg_sns_categories__profitero_winn') }} as SRC 
                        {% if is_incremental() %}
                              where src.load_dts > (select dateadd('HOUR',-1,max(load_dts)) from {{ this }})
                            {% endif %}   )

/*
SRC_CATWINN        as ( SELECT * FROM STAGING.v_psa_stg_sns_categories__profitero_winn )
*/
---- LOGIC LAYER ----
