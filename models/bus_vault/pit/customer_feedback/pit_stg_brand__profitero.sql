...
SRC_SB             as ( SELECT BRAND, BRAND_HK, FULL_NAME, ID, OWNER, SUBBRAND, SUBSUBBRAND FROM {{ ref('sat_brand__profitero') }} as SRC 
                        qualify 1= row_number() over(partition by BRAND_HK order by UPDATED_AT DESC) )

/*
SRC_HB             as ( SELECT * FROM RAW_VAULT.HUB_BRAND_V2 )
SRC_SB             as ( SELECT * FROM RAW_VAULT.SAT_BRAND__PROFITERO )
*/
---- LOGIC LAYER ----
