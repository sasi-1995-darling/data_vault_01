...
with
--sales
cte_sat_sales_moen_inc__amazon_fivetran as (
 select * from   
    {{ ref('sat_sales_moen_inc__amazon_fivetran') }}    

)
, cte_sat_sales_moen_inc__amazon_fivetran_latest as (
    select * from cte_sat_sales_moen_inc__amazon_fivetran
...
,cte_sat_sales_moen_ana__amazon_fivetran as (
 select * from   
    {{ ref('sat_sales_moen_ana__amazon_fivetran') }}    

)
, cte_sat_sales_moen_ana__amazon_fivetran_latest as (
    select * from cte_sat_sales_moen_ana__amazon_fivetran
    qualify row_number() over(partition by report_end_date, asin, vendorcentral_account,customer_returns order by load_dts desc)=1
)
, cte_sat_sales_moen__amazon_fivetran_latest as (
    select * from cte_sat_sales_moen_inc__amazon_fivetran_latest
    union all
    select * from cte_sat_sales_moen_ana__amazon_fivetran_latest
)
--inventory
, cte_sat_inventory_moen_inc__amazon_fivetran as (
 select * from   
    {{ ref('sat_inventory_moen_inc__amazon_fivetran') }} 

)
, cte_sat_inventory_moen_inc__amazon_fivetran_latest as (
    select * from cte_sat_inventory_moen_inc__amazon_fivetran
    qualify row_number() over(partition by report_end_date, asin, vendorcentral_account order by load_dts desc)=1
)
, cte_sat_inventory_moen_ana__amazon_fivetran as (
 select * from   
    {{ ref('sat_inventory_moen_ana__amazon_fivetran') }} 

)
, cte_sat_inventory_moen_ana__amazon_fivetran_latest as (
    select * from cte_sat_inventory_moen_ana__amazon_fivetran
    qualify row_number() over(partition by report_end_date, asin, vendorcentral_account order by load_dts desc)=1
)
, cte_sat_inventory_moen__amazon_fivetran_latest as (
    select * from cte_sat_inventory_moen_inc__amazon_fivetran_latest
    union all
    select * from cte_sat_inventory_moen_ana__amazon_fivetran_latest
)
, cte_sat_inventory_moen__amazon_fivetran_converted_weekly as (
    select
          i.STORE_HK
        , i.LOAD_DTS
        , dateadd(
        ...
        , i.BKCC
        , i.HASHDIFF
    from cte_sat_inventory_moen__amazon_fivetran_latest i
...
, cte_moen_amazon_fivetran_data as (
    select 
    ...
    from cte_sat_sales_moen__amazon_fivetran_latest s
    full join
    cte_sat_inventory_moen__amazon_fivetran_converted_weekly i
    on 
...
from {{ ref('hub_store') }} as hcl
    inner join cte_moen_amazon_fivetran_data as p
