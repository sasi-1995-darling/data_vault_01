...
with 
cte_sat_sales_sentrysafe__amazon_fivetran as (
 select * from   
    {{ ref('sat_sales_sentrysafe__amazon_fivetran') }}    

)
, cte_sat_sales_sentrysafe__amazon_fivetran_latest as (
    select * from cte_sat_sales_sentrysafe__amazon_fivetran
...
, cte_sat_inventory_sentrysafe__amazon_fivetran as (
 select * from   
    {{ ref('sat_inventory_sentrysafe__amazon_fivetran') }} 

)
, cte_sat_inventory_sentrysafe__amazon_fivetran_latest as (
    select * from cte_sat_inventory_sentrysafe__amazon_fivetran
    qualify row_number() over(partition by report_end_date, asin, vendorcentral_account order by load_dts desc)=1
)
, cte_sat_inventory_sentrysafe__amazon_fivetran_converted_weekly as (
    select
          i.STORE_HK
        , i.LOAD_DTS
        , dateadd(
        ...
        , i.BKCC
        , i.HASHDIFF
    from cte_sat_inventory_sentrysafe__amazon_fivetran_latest i
...
, cte_sentrysafe_amazon_fivetran_data as (
    select 
    ...
    from cte_sat_sales_sentrysafe__amazon_fivetran_latest s
    full join
    cte_sat_inventory_sentrysafe__amazon_fivetran_converted_weekly i
    on 
...
from {{ ref('hub_store') }} as hcl
    inner join cte_sentrysafe_amazon_fivetran_data as p
        on hcl.store_hk = p.store_hk
)
--End of merging sales, inventory with hub_store

-- Start of applying pos logic/ref to sentrysafe
