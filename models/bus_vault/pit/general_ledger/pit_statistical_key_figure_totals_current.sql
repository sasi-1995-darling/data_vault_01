---- SRC LAYER ----
WITH
SRC_LSKFT          as ( SELECT * FROM {{ ref('lnk_statistical_key_figure_totals') }} as SRC  ),
...
SRC_HTF            as ( SELECT * FROM {{ ref('hub_tracing_factor') }} as SRC  ),
...
SRC_SAT_WINN       as ( SELECT * FROM {{ ref('lsat_statistical_key_figure_totals__winn_sap') }} as SRC  
    qualify 1= row_number() over (partition by STATISTICAL_KEY_FIGURE_TOTALS_HK order by LOAD_DTS DESC))

/*
SRC_LSKFT          as ( SELECT * FROM RAW_VAULT.lnk_statistical_key_figure_totals)
...
, SRC_HTF            as ( SELECT * FROM RAW_VAULT.hub_tracing_factor )
...
, SRC_SAT_WINN       as ( SELECT * FROM RAW_VAULT.lsat_statistical_key_figure_totals__winn_sap )
*/
---- LOGIC LAYER ----
...
, LOGIC_HTF as (
    SELECT
        TRACKING_FACTOR_HK                                           as                             HUB_TRACKING_FACTOR_HK
      , TRACKING_FACTOR_BK
    FROM SRC_HTF
)
...
, LOGIC_SAT_WINN as (
    SELECT
        STATISTICAL_KEY_FIGURE_TOTALS_HK                             as          SAT_WINN_STATISTICAL_KEY_FIGURE_TOTALS_HK
      , LEDNR                                                        as                     LEDGER_FOR_CONTROLLING_OBJECTS
      , OBJNR                                                        as                                         OBJECT_NUM
      , CAST(GJAHR AS NUMBER)                                        as                                  FISCAL_YEAR__YYYY
      , WRTTP                                                        as                                         VALUE_TYPE
      , VERSN                                                        as                                            VERSION
      , STAGR                                                        as                             STATISTICAL_KEY_FIGURE
      , HRKFT                                                        as                                   CO_KEY_SUBNUMBER
      , VRGNG                                                        as                            CO_BUSINESS_TRANSACTION
      , PERBL                                                        as                                       PERIOD_BLOCK
      , SME001                                                       as                                  STATISTICAL_QTY_1
      , SME002                                                       as                                  STATISTICAL_QTY_2
      , SMA001                                                       as                              MAX_STATISTICAL_QTY_1
      , SMA002                                                       as                              MAX_STATISTICAL_QTY_2
      , PSA_DELETE_IND
    FROM SRC_SAT_WINN
)
---- RENAME LAYER ----
...
, RENAME_SAT_WINN as (
    SELECT
        SAT_WINN_STATISTICAL_KEY_FIGURE_TOTALS_HK
...
, JOIN_RESULT as (
    ...
    LEFT JOIN FILTER_HCO
        ON FILTER_LSKFT.OBJECT_NUMBER_HK = FILTER_HCO.HUB_OBJECT_NUMBER_HK
    LEFT JOIN FILTER_HCVT
        ON FILTER_LSKFT.COST_VALUE_TYPE_HK = FILTER_HCVT.HUB_COST_VALUE_TYPE_HK
    LEFT JOIN FILTER_HAFP
        ON FILTER_LSKFT.FISCAL_PERIOD_HK = FILTER_HAFP.HUB_FISCAL_PERIOD_HK
    LEFT JOIN FILTER_HTF
        ON FILTER_LSKFT.TRACKING_FACTOR_HK = FILTER_HTF.HUB_TRACKING_FACTOR_HK
    LEFT JOIN FILTER_HCV
        ON FILTER_LSKFT.COST_VERSION_HK = FILTER_HCV.HUB_COST_VERSION_HK
    LEFT JOIN FILTER_HL
        ON FILTER_LSKFT.LEDGER_HK = FILTER_HL.HUB_LEDGER_HK
    LEFT JOIN FILTER_HCBTT
        ON FILTER_LSKFT.COST_TRANSACTION_TYPE_HK = FILTER_HCBTT.HUB_COST_TRANSACTION_TYPE_HK
    LEFT JOIN FILTER_SAT_WINN
        ON FILTER_LSKFT.STATISTICAL_KEY_FIGURE_TOTALS_HK = SAT_WINN_STATISTICAL_KEY_FIGURE_TOTALS_HK
)

---- FINAL LAYER ----
SELECT
          'PIT_STATISTICAL_KEY_FIGURE_TOTALS'                          as PIT_REC_SRC
