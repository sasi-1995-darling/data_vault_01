---- SRC LAYER ----
WITH
SRC_SLR            as ( SELECT * FROM {{ ref('v_psa_stg_prod_grp_lu__lrsn_psft') }} as SRC 
                         {% if is_incremental() %}
                         WHERE SRC.LOAD_DTS > (SELECT DATEADD('HOUR', '-1', MAX(LOAD_DTS)) FROM {{this}})
                         {% endif %}  )

/*
SRC_SLR            as ( SELECT * FROM STAGING.v_psa_stg_prod_grp_lu__LRSN_PSFT  )
*/
---- LOGIC LAYER ----
...
WHERE NOT EXISTS (
    SELECT 1 
    FROM {{ this }} existing
    WHERE existing.PROD_GRP_BK = JOIN_RESULT.PROD_GRP_BK
...
qualify 1= row_number()over(partition by PROD_GRP_BK, HASHDIFF order by PSA_LOAD_DTS)
union all
    SELECT          GR.VALUE as PROD_GRP_BK
    , null as L_GRP_TYPE
    , null as L_GRP_CODE
    , null as SETID
    , null as DESCR
