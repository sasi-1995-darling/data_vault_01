...
SRC_s_dp           as ( SELECT BIC_Z9ADFCST, BIC_ZANALIFT, BIC_ZCUSTINV, BIC_ZFINLFCST, BIC_ZFQUARTER, BIC_ZLEADIND, BIC_ZMCORHIST, BIC_ZMOENPROP, BIC_ZNEWPFCST, BIC_ZOUTTED, BIC_ZPOSQTY, BIC_ZPRMOFCST, BIC_ZPROMOQTY, BIC_ZSTATFCST, BIC_ZTRANSDMD, BIC_ZZAPOACCT, BIC_ZZPLANPER, CALMONTH, CALQUARTER, CALWEEK, CALYEAR, DATE0, DEMAND_PLAN_LHK, DISTR_CHAN, DIVISION, FISCPER, FISCVARNT, 
                                GLCHANGETIME, MATERIAL, ORDER_QTY, PLANT, LOAD_DTS ,
                                PSA_DELETE_IND , SALESORG, MAX(TO_DATE(BIC_ZZPLANPER, 'MONYY')) over() as ltst_plan_prd -- pulling the latest BIC_ZZPLANPER value
                                FROM {{ ref('lmsat_demand_plan__winn_sap') }} as SRC
                      ),
...
SRC_s_dp           as ( SELECT * FROM RAW_VAULT.lmsat_demand_plan__winn_sap )
...
, LOGIC_s_dp as (
    SELECT
      ...
      , LOAD_DTS
        FROM SRC_s_dp b
        where TO_DATE(BIC_ZZPLANPER, 'MONYY') = ltst_plan_prd -- Isolate the max plan period records
        QUALIFY 1= ROW_NUMBER() OVER (PARTITION BY DEMAND_PLAN_LHK, CALWEEK ORDER BY GLCHANGETIME DESC) -- after isolating max plan period records it is pulling the latest version of record. 
)
...
SELECT
        ...
        , ACQUISITION_DATE__YYYYMMDD
        , LOAD_DTS
        , 'PB_DEMAND_PLAN_CURRENT'                                       as PB_REC_SRC
