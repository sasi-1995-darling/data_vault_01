...
SRC_lnk as ( 
    SELECT 
        ...
        ,STORE_HK
        ,REC_SRC 
    FROM {{ ref('lnk_channel_traffic_items') }} 
),
SRC_hub_item as ( 
    SELECT 
         ITEM_BK
...
SRC_sat_vc as ( 
    SELECT 
        ...
        , PSA_DELETE_IND
        , load_dts
    FROM {{ ref('lmsat_channel_traffic_items__amazon_vc') }} 
    where report_end_date between '2023-01-01' and '2025-02-10' and vendorcentral_account = 'Moen Inc'
    qualify 1 = row_number() over(partition by CHANNEL_TRAFFIC_ITEM_LHK order by load_dts DESC) 
),
SRC_sat_anaheim as ( 
    SELECT 
          CHANNEL_TRAFFIC_ITEM_LHK
        ...
        , PSA_DELETE_IND
        , load_dts
    FROM {{ ref('lmsat_channel_traffic_items__amazon_moen_anaheim') }} 
...
SRC_sat_inc as ( 
    SELECT 
        ...
        , PSA_DELETE_IND
        , load_dts
    FROM {{ ref('lmsat_channel_traffic_items__amazon_moen_inc') }} 
    where end_date >= '2025-02-11'
    qualify 1 = row_number() over(partition by CHANNEL_TRAFFIC_ITEM_LHK order by load_dts DESC) 
)
...
, LOGIC_lnk as (
    SELECT
        ...
        ,STORE_HK
        ,REC_SRC
        ,'PB_VENDOR_TRAFFIC_ITEMS' as PB_REC_SRC
...
, LOGIC_sat_vc as (
    SELECT
         CHANNEL_TRAFFIC_ITEM_LHK as LNK_HK
        ,ASIN
        ,GLANCE_VIEWS
...
, JOIN_RESULT as (
    ...
    LEFT JOIN UNIFIED_sat sat      ON l.CHANNEL_TRAFFIC_ITEM_LHK = sat.LNK_HK
)
