...
SRC_STI            as ( SELECT ANONYMOUS_ID, CONSUMER_HK, CONTEXT_APP_NAME, CONTEXT_OS_NAME, CONTEXT_TRAITS_ANONYMOUS_ID, EVENT, EVENT_TEXT, REC_SRC, TIMESTAMP FROM {{ ref('sat_user_engagement_track__ios_moen') }} as SRC 
                        WHERE REC_SRC = 'US.IOS_MOEN.TRACKS'
...
SRC_STI            as ( SELECT * FROM raw_vault.SAT_USER_ENGAGEMENT_TRACK__IOS_MOEN )
...
, LOGIC_STI as (
    SELECT
        CONSUMER_HK                                                  as                                    STI_CONSUMER_HK
...
, JOIN_RESULT as (
    SELECT *
    FROM FILTER_HUE
    INNER JOIN FILTER_STI
        ON CONSUMER_HK = STI_CONSUMER_HK
