...
SRC_lsea           as ( SELECT CURRENCY_CODE, LNK_ECOMMERCE_ACCOUNT_PROPERTY_HK, NAME, PROPERTY_TYPE, REC_SRC, SERVICE_LEVEL, TIME_ZONE 
                        FROM {{ ref('lsat_ecommerce_account_property__winn_google_analytics') }} as SRC  ),
SRC_sea            as ( SELECT ECOMMERCE_ACCOUNT_HK, REGION_CODE FROM {{ ref('sat_ecommerce_account__winn_google_analytics') }} as SRC  ),
SRC_septc          as ( SELECT DATE, ECOMMERCE_PROPERTY_HK, ENGAGED_SESSIONS, ENGAGEMENT_RATE, EVENT_COUNT, KEY_EVENTS, SESSIONS, SESSION_DEFAULT_CHANNEL_GROUPING, 
                        TOTAL_REVENUE, TOTAL_USERS FROM {{ ref('sat_ecommerce_property_traffic_by_channel__winn_google_analytics') }} as SRC 
                        QUALIFY 1 = ROW_NUMBER() OVER (PARTITION BY ECOMMERCE_PROPERTY_HK, SESSION_DEFAULT_CHANNEL_GROUPING, DATE ORDER BY LOAD_DTS DESC) ),
...
SRC_lsea           as ( SELECT * FROM raw_vault.lsat_ecommerce_account_property__winn_google_analytics )
SRC_sea            as ( SELECT * FROM raw_vault.sat_ecommerce_account__winn_google_analytics )
SRC_septc          as ( SELECT * FROM raw_vault.sat_ecommerce_property_traffic_by_channel__winn_google_analytics )
...
, LOGIC_lsea as (
    SELECT
      ...
      , REC_SRC
      , LNK_ECOMMERCE_ACCOUNT_PROPERTY_HK                            as             LSEA_LNK_ECOMMERCE_ACCOUNT_PROPERTY_HK
    FROM SRC_lsea
)
...
, LOGIC_septc as (
    SELECT
      ...
      , EVENT_COUNT
      , TOTAL_REVENUE
      , ECOMMERCE_PROPERTY_HK                                        as                        SEPTC_ECOMMERCE_PROPERTY_HK
      , DATE                                                         as                                         SEPTC_DATE
    FROM SRC_septc
)
...
, RENAME_lsea as (
    SELECT
      ...
      , REC_SRC
      , LSEA_LNK_ECOMMERCE_ACCOUNT_PROPERTY_HK
    FROM LOGIC_lsea
)
...
, JOIN_RESULT as (
    ...
    INNER JOIN FILTER_lsea
        ON FILTER_leap.LNK_ECOMMERCE_ACCOUNT_PROPERTY_HK = LSEA_LNK_ECOMMERCE_ACCOUNT_PROPERTY_HK
    INNER JOIN FILTER_sea
        ON FILTER_leap.ECOMMERCE_ACCOUNT_HK = SEA_ECOMMERCE_ACCOUNT_HK
    INNER JOIN FILTER_septc
        ON FILTER_leap.ECOMMERCE_PROPERTY_HK = SEPTC_ECOMMERCE_PROPERTY_HK
    LEFT JOIN FILTER_pdtf
        ON SEPTC_DATE = PDTF_DATE
)
