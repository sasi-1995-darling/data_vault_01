...
SRC_SRBY           as ( SELECT * FROM {{ ref('sat_review__bazaarvoice_yale') }} as SRC 
...
SRC_SRBY           as ( SELECT * FROM raw_vault.SAT_REVIEW__BAZAARVOICE_YALE )
...
, LOGIC_SRBY as (
    SELECT
      ...
      , ''                                                           as                                         REVIEW_URL
      , CONCAT('YALE-',TO_VARCHAR(ID))                               as                                               UKEY
      ...
      , 'FALSE'                                                      as                                         IS_DELETED
      , CONCAT('YALE-',TO_VARCHAR(ID))                               as                                            SRBY_ID
    FROM SRC_SRBY
)
...
, JOIN_RESULT as (
    SELECT *
    FROM FILTER_HP
    INNER JOIN FILTER_SRBY
        ON HP_PRODUCT_HK = PRODUCT_HK
    LEFT JOIN FILTER_SP
        ON SRBY_ID = REVIEW_ID
)
