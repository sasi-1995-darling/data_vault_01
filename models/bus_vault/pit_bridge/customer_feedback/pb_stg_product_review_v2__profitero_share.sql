...
SRC_LPRP           as ( SELECT * FROM {{ ref('lsat_product_review__profitero_share') }} as SRC 
                        WHERE 1=1
                        qualify 1= row_number() over(partition by CUSTOMER_PRODUCT_ID, RETAILER_Id, product_id, UKEY order by UPDATED_AT DESC) ),
SRC_LPB            as ( SELECT BRAND_HK, LOAD_DTS, PRODUCT_HK FROM {{ ref('lnk_product_brand') }} as SRC  ),
...
SRC_LPRP           as ( SELECT * FROM raw_vault.LSAT_PRODUCT_REVIEW__PROFITERO_SHARE )
